...
Ext_define('ReExtExample.view.grid.grouped.GridController', {
  ...
  ageRenderer: function (value, record, dataIndex, cell, column) {
    if (record && value) {
  ...
  onShowFilters: function (menu) {
    ...
    for (j in filters) {
      ...
      if (currFilter.property) {
        ...
        filterItem = menu.add({
          text: colMap[currFilter.property].name + ': ' + currFilter.operator + ' ' + value,
          checked: true,
  ...
  addFilter: function (currFilter) {
    ...
    for (j in inFilters) {
      ...
      if (
        inCurrFilter.property === currFilter.property &&
  ...
  removeFilter: function (currFilter) {
    ...
    for (j in inFilters) {
      ...
      if (
        !(
          currFilter.property === inCurrFilter.property &&
